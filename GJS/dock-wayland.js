...
const HyprCandyDock = GObject.registerClass({
    ...
    constructor(app) {
        ...
        // Make it a proper dock after creation
        this.makeDock();
    }
    
    makeDock() {
        // Try to use Hyprland rules to make it dock-like
        try {
            const proc = new Gio.Subprocess({
                argv: ['hyprctl', 'keyword', 'windowrule', 'float, title:HyprCandy Dock'],
                flags: Gio.SubprocessFlags.NONE
            });
            proc.init(null);
            
            const proc2 = new Gio.Subprocess({
                argv: ['hyprctl', 'keyword', 'windowrule', 'noborder, title:HyprCandy Dock'],
                flags: Gio.SubprocessFlags.NONE
            });
            proc2.init(null);
            
            const proc3 = new Gio.Subprocess({
                argv: ['hyprctl', 'keyword', 'windowrule', 'noshadow, title:HyprCandy Dock'],
                flags: Gio.SubprocessFlags.NONE
            });
            proc3.init(null);
            
            console.log('🔧 Applied Hyprland dock rules');
        } catch (e) {
            console.log('⚠️ Could not apply Hyprland rules:', e);
        }
    }
    ...
    setupWindow() {
        ...
        // Make it dock-like (GTK4 compatible)
        this.set_decorated(false);
        this.set_resizable(false);
        // Note: set_skip_taskbar_hint and set_skip_pager_hint don't exist in GTK4
        
        // Present first to get the surface
        this.present();
        
        // Position at bottom using move (for X11) or accept default (for Wayland)
        try {
            this.move(
    ...
    setupStyle() {
        ...
        // Try to load matugen CSS, but fallback if not available
        let cssContent = `
            window {
        ...
        // Try to load matugen colors if available
        try {
            const matugenFile = Gio.File.new_for_path('/home/king/.hyprcandy/.config/gtk-4.0/gtk.css');
            if (matugenFile.query_exists(null)) {
                cssContent = `
                    @import url('/home/king/.hyprcandy/.config/gtk-4.0/gtk.css');
                    ...
                    .dock-item.running:hover {
                        ...
                        border-color: alpha(@primary, 0.8);
                    }
                    ...
                    .running-indicator {
                        ...
                        width: 8px;
                        height: 8px;
                        box-shadow: 0 2px 4px alpha(@success_bg_color, 0.4);
                    }
                    
                    .launcher-button {
                        background: linear-gradient(135deg, alpha(@primary, 0.1), alpha(@primary, 0.05));
                        border: 1px solid alpha(@primary, 0.2);
                    }
                    
                    .launcher-button:hover {
                        background: linear-gradient(135deg, alpha(@primary, 0.2), alpha(@primary, 0.1));
                    ...
                    image {
                        -gtk-icon-palette: @primary;
                    }
                `;
                console.log('🎨 Using matugen colors');
            }
        } catch (e) {
