...
const HyprCandyDock = GObject.registerClass({
    ...
    setupWindow() {
        ...
        // Get the surface for proper positioning
        const surface = this.get_surface();
        if (surface) {
            // Use Wayland-specific positioning if available
            if (surface.is_wayland) {
                // For Wayland, we need to use layer shell
                console.log('🪟 Wayland detected, using surface positioning');
            }
            
            // Try to position at bottom
            try {
                // Alternative positioning approach
                const rect = new Gdk.Rectangle({
                    x: geometry.x + (geometry.width - dockWidth) / 2,
                    y: geometry.y + geometry.height - dockHeight - 20,
                    width: dockWidth,
                    height: dockHeight
                });
                
                // This might work for some window managers
                if (surface.move_to_rect) {
                    surface.move_to_rect(rect, Gdk.Gravity.SOUTH, Gdk.Gravity.SOUTH, true);
                }
            } catch (e) {
                console.log('⚠️ Could not set custom position, using default');
            }
        }
